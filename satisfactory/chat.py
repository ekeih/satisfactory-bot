...
class Bot:
    ...
    def __init__(self, bot_token: str, chat_id: int, github_client: satisfactory.git.Git or None, youtube_client: satisfactory.youtube.Youtube or None):
    ...
    async def check_images(self, context: CallbackContext) -> None:
        ...
        if new_versions_count == 0 and not context.job:
            ...
            await context.bot.send_message(chat_id=self.chat_id, text="No newer versions available, doing nothing.\n\n%s" % (satisfactory.game.get_random_quote()), parse_mode=ParseMode.HTML)
    ...
    async def check_news(self, context: CallbackContext) -> None:
        ...
        for news in recent_news:
            if len(news) <= constants.MessageLimit.MAX_TEXT_LENGTH:
                await context.bot.send_message(chat_id=self.chat_id, text=news, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
            else:
                parts = []
                while len(news) > 0:
                    if len(news) > constants.MessageLimit.MAX_TEXT_LENGTH:
                        ...
                        break
                for part in parts:
                    await context.bot.send_message(chat_id=self.chat_id, text=part, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
        if len(recent_news) == 0 and not context.job:
            ...
            await context.bot.send_message(chat_id=self.chat_id, text="No recent news found.\n\n%s" % (satisfactory.game.get_random_quote()), parse_mode=ParseMode.HTML)
    ...
    async def send_random_quote(self, context: CallbackContext) -> None:
        await context.bot.send_message(chat_id=self.chat_id, text=satisfactory.game.get_random_quote(), parse_mode=ParseMode.HTML)
